...
# Analyses were completed in Cm_CJS_v3_Mark.Rmd
...
I considered several possible CJS models for this analysis. For survival rates, they were treated as either constant, affected by transients (time-since-marking; TSM), a function of age class (immature < 81.6 cm CCL) or a function of size (CCL) at first capture. For capture probabilities, they were treated as either constant, time dependent, different between the first and subsequent captures (trap response), or a function of effort. A total of 14 models were fitted to the data (Table 1).
...
knitr::kable(mark.table %>% select(ID, Phi, p, Model) %>% arrange(by = as.numeric(ID)), 
             digits = 2,
             caption = "Table 1. Definitions of 14 CJS models fitted to CMR data of green turtles. '~1' indicates constant.",
...
BKS.models.df <- model.table(Cm.results) %>% select(Phi, p, model, DeltaAICc) %>% 
...
knitr::kable(BKS.models.df, 
             digits = 2,
             col.names = c("Phi", "p", "model", "DeltaAICc"),
...
BMA.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
...
GNO.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
...
IES.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
...
LSI.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
...
MUL.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
...
PAO.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
...
PLM.models.df <- model.table(Cm.results)%>% select(Phi, p, model, DeltaAICc) %>% 
  mutate(model = paren2bracket(model))
```
