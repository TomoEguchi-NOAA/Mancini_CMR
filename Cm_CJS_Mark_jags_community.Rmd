...
Using the best model (```r models.MARK.old %>% filter(community == loc) %>% select(model)```), the estimated mean survival rate was ```r signif(N.Phi.hats$mean.phi[2, "mean"], 3)``` (SD = ```r signif(N.Phi.hats$Phihats[2, "sd"], 3)```, 95% CI = [```r signif(N.Phi.hats$Phihats[2, "lcl"], 3)``` - ```r signif(N.Phi.hats$Phihats[2, "ucl"], 2)```]).
...
Using the best model (```r models.MARK.old %>% filter(community == loc) %>% select(model)```), the estimated survival rate was ```r signif(N.Phi.hats$mean.phi[2, "mean"], 3)``` (mean, SD = ```r signif(N.Phi.hats$Phihats[2, "sd"], 3)```, 95% CI = [```r signif(N.Phi.hats$Phihats[2, "lcl"], 3)``` - ```r signif(N.Phi.hats$Phihats[2, "ucl"], 2)```]).
...
Although the Phidot model was the best according to the AIC values, we used PhiTSM in the Bayesian inference because the estimated survival rate from the Phi[.] model was very small (```r signif(N.Phi.hats$mean.phi[2, "mean"], 3)```). 
