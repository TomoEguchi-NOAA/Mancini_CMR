...
MLE appeared to overestimate some years. Precision of abundance estimates were generally better in the Bayesian approach than for that of MLEs. It is likely that a large portion of turtles in this area was transients. 
...
N.Phi.p.hats.Mark.1 <- extract.Nhats(Cm.inputs, Cm.results.Mark, real.estimates.1)
N.Phi.p.hats.Mark.2 <- extract.Nhats(Cm.inputs, Cm.results.Mark, real.estimates.2)
...
```{r parameter_estimates2_GNO, echo=F, warning=FALSE, message=FALSE}
plot.Nhats.Mark(loc, 
                Model.ID = "11",
                N.Phi.p.hats.Mark.2, 
                save.fig, fig.height, fig.width)
...
```{r plot-Nhats2-GNO, echo=FALSE, cache=TRUE, warning = FALSE, message = FALSE, fig.cap="Estimated abundance of green turtles at GNO when the maximum likelihood approach and the Phi[~1]p[t] model were used to analyze capture histories. Filled circles indicate means and error bars indicate 95% confidence intervals (CI)."}
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_", loc, "_M11.png"))
```
...
Bayesian estimates were more precise than those of MLEs in general. 
...
CI_IES.model.comp <- model.table(Cm.results.Mark)
CI_IES.model.comp$Model <- paren2bracket(CI_IES.model.comp$model)

knitr::kable(data.frame(ID = rownames(CI_IES.model.comp),
                        model = CI_IES.model.comp$Model,
                        deltaAICc = CI_IES.model.comp %>% select(DeltaAICc)), 
             digits = 2,
             col.names = c("ID", "model", "$\\Delta$AICc"),
             escape = FALSE,
             row.names = F,
...
real.estimates.1 <- Cm.results.Mark$Phi.tsm.p.effort$results$real %>% 
  rownames_to_column("parameter")

real.estimates.2 <- Cm.results.Mark$Phi.tsm.p.tsm.effort$results$real %>% 
  rownames_to_column("parameter")

effort.phats <- data.frame(effort = unique(Cm.inputs$effort$effort),
                           phat = real.estimates.1[grep("p", real.estimates.1$parameter),])

Cm.inputs$effort %>% 
  left_join(effort.phats, by = "effort") %>%
  left_join(data.frame(season = colnames(Cm.inputs$CJS.data$data),
                       n.caught = colSums(Cm.inputs$CJS.data$data)),
            by = "season") -> p.hats.all

# p.hats.Mark <- list(Nhats = p.hats.all)
N.Phi.p.hats.Mark.1 <- extract.Nhats(Cm.inputs, Cm.results.Mark, real.estimates.1)
# N.Phi.p.hats.Mark.2 <- extract.Nhats(Cm.inputs, Cm.results.Mark, real.estimates.2)
...
Using the best model (Phi[tsm]p[effort], Table \@ref(tab:table-models-IES)), the estimated mean survival rate of residents was `r signif(N.Phi.p.hats.Mark.1$Phihats[1,"estimate"], 3)` (SE = `r signif(N.Phi.p.hats.Mark$Phihats[1, "se"], 3)`, 95% CI = [`r signif(N.Phi.p.hats.Mark$Phihats[1, "lcl"], 2)` - `r signif(N.Phi.p.hats.Mark$Phihats[1, "ucl"], 2)`]). 
...
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_", loc, "_M9.png"))
```
...
# v5 means the same data and model as what Mark selected
N.Phi.p.hats.Jags <- stats.Bayesian_v5(rownames(CI_IES.model.comp)[1], 
                                       Phi.spec = CI_IES.model.comp[1, "Phi"],
                                       p.spec = CI_IES.model.comp[1, "p"],
                                       loc)
...
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_", loc, "_M9_jags_v5.png"))
```


```{r Nhats_comparison_IES, echo=F, include=F}
#Abundance estimates between the two models were similar?
Nhats_comparison(loc, rownames(CI_IES.model.comp)[1], 
                 N.Phi.p.hats.Jags$Model.ID,
                 N.Phi.p.hats.Mark.1, 
                 N.Phi.p.hats.Jags, save.fig,
                 fig.height, fig.width)
...
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_comp_", loc, "_M9_M9.png"))
```

#### MUL {-}

Probably not useful to include this site because of few recaptures.
...
MUL.model.comp <- model.table(Cm.results.Mark)
MUL.model.comp$Model <- paren2bracket(MUL.model.comp$model)

knitr::kable(data.frame(ID = rownames(MUL.model.comp),
                        model = MUL.model.comp$Model,
                        deltaAICc = MUL.model.comp %>% select(DeltaAICc)), 
             digits = 2,
             col.names = c("ID", "model", "$\\Delta$AICc"),
             escape = FALSE,
             row.names = F,
...
real.estimates.1 <- Cm.results.Mark$Phi.dot.p.dot$results$real %>% 
  rownames_to_column("parameter")

real.estimates.2 <- Cm.results.Mark$Phi.dot.p.effort$results$real %>% 
  rownames_to_column("parameter")
...
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_", loc, "_M1.png"))
```
...
N.Phi.p.hats.Jags <- stats.Bayesian_v5(rownames(MUL.model.comp)[1], 
                                       Phi.spec = MUL.model.comp[1, "Phi"],
                                       p.spec = MUL.model.comp[1, "p"],
                                       loc)
...
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_", loc, "_M1_jags_v5.png"))
```


```{r Nhats_comparison_MUL, echo=F, include=F}
#Abundance estimates between the two models were similar?
Nhats_comparison(loc, rownames(MUL.model.comp)[1], 
                 N.Phi.p.hats.Jags$Model.ID,
                 N.Phi.p.hats.Mark.1, 
                 N.Phi.p.hats.Jags, save.fig,
                 fig.height, fig.width)
...
knitr::include_graphics(paste0("figures/Cm_2023_Nhats_comp_", loc, "_M1_M1.png"))
```
...
PAO.model.comp <- model.table(Cm.results.Mark)
PAO.model.comp$Model <- paren2bracket(PAO.model.comp$model)

knitr::kable(data.frame(ID = rownames(PAO.model.comp),
                        model = PAO.model.comp$Model,
                        deltaAICc = PAO.model.comp %>% select(DeltaAICc)), 
             digits = 2,
             col.names = c("ID", "model", "$\\Delta$AICc"),
             escape = FALSE,
             row.names = F,
...
# v5 means the same data and model as what Mark selected
N.Phi.p.hats.Jags <- stats.Bayesian_v5(rownames(PAO.model.comp)[1], 
                                       Phi.spec = PAO.model.comp[1, "Phi"],
                                       p.spec = PAO.model.comp[1, "p"],
                                       loc)
...
#Abundance estimates between the two models were similar?
Nhats_comparison(loc, rownames(PAO.model.comp)[1], 
                 N.Phi.p.hats.Jags$Model.ID,
                 N.Phi.p.hats.Mark.1, 
                 N.Phi.p.hats.Jags, save.fig,
                 fig.height, fig.width)
