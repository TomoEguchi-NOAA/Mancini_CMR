...
All tests indicate that almost all datasets passed the GOF tests. (except PAO Test2.ct AND BKS for test2.CL, which had p < 0.05). Possible reasons for these possible violations are discussed later.  
...
knitr::kable(data.frame(ID = rownames(BKS.model.comp),
             ...
             digits = 2,
             col.names = c("ID", "model", "DeltaAICc"),
             row.names = F,
...
Because the difference in AICc values between the two top models (M4 and M11) was `r signif(BKS.model.comp[2,"DeltaAICc"], 3)`, I model-averaged the abundance estimates between the two models, where the original weights of these models including 12 other models were `r signif(BKS.model.comp[1,"weight"], 3)` and `r signif(BKS.model.comp[2, "weight"], 3)`, respectively. But... because of the questionable survival estimates from the second best model (above), I don't think we should be averaging the estimates. 
