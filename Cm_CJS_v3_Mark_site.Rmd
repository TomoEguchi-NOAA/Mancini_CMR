...
2024-12-16: Something is not quite right with data extraction and CJS input data creation. Double check the steps. 

Bring in the data file and get ready for CJS. Some communities did not have large sample sizes (the number of captured turtles) in the starting phase. This caused some difficulties in estimating capture/recapture probabilities. So, after some frustrating days, I decided to remove these sampling periods, specifically for the beginning of each dataset. This had to be done manually. Selection of starting capture seasons is done in Cm_recaptures_Sept2024.Rmd:
...
# In Sept 2024, we decided to use "Macro_site" as the grouping variable because
# turtles are found in nearby capture areas (email on 2024-09-26)
...
# Selection of starting capture seasons is done in Cm_recaptures_Sept2024.Rmd

for (k in 1:length(site.names.used)){
  
  if (!file.exists(paste0("RData/CJS_Cm_2023_RMark_", site.names.used[k], ".rds"))){

    dat.1.Cm.site <- filter(dat.1.Cm, 
                            Macro_site == site.names.used[k]) 
    CJS.data <- dat2CJS(dat.1.Cm.site, 
                        save.file = FALSE)
    n.GT2.cap <- length(which(rowSums(CJS.data$data) > 1))
    p.recap[k] <- n.GT2.cap/nrow(CJS.data$data)
    
    # This has been converted into a function and moved to Mancini_functions.R
    # so that other scripts can use it.
    # first.sample <- switch(site.names.used[k],
    #                        "BKS" = "2015-01",
    #                        "CL_BMA" = "2001-08",
    #                        "GNO" = "2001-08",
    #                        "CI_IES" = "2006-01",
    ...
    #                        "MUL" = "2008-08",
    #                        "PAO" = "2001-08")
    # -01 is from Oct of previous year to Apr 30
    # -08 is from May 1 to Sep 30
    # year <- unlist(str_split(first.sample, "-") )[1] %>% as.numeric()
    # season.begin <- unlist(str_split(first.sample, "-") )[2]
    # first.sample.date <- as.Date(ifelse(season.begin == "01",
    #                                     as.Date(paste0(year-1, "-10-01")),
    #                                     as.Date(paste0(year, "-05-01"))),
    #                              format = "%Y-%m-%d") 
    
    first.sample. <- first.sample(site.names.used[k])    
    
    dat.2.Cm.site <- filter(dat.1.Cm.site, 
                            DATE >= first.sample.$first.sample.date)
    
    CJS.data <- dat2CJS(dat.2.Cm.site, save.file = FALSE)
    #stop()
    ...
    dat.1.Cm.site %>% select(season, "DATE") %>% 
    ...
    begin.year <- year(min(dat.1.Cm.site$DATE, na.rm = T))
    dp <- process.data(CH, 
                       model = "CJS", 
    ...
    sum_win <- data.frame(sum_win = unlist(ifelse(tmp3 == "08", 
                                                  "summer", 
                                                  "winter")) [1:length(levels(ddl$p$time))],
    ...
    sum_win <- data.frame(sum_win = unlist(ifelse(tmp3 == "08", 
                                                  "summer", 
                                                  "winter")) [1:length(levels(ddl$Phi$time))],
    ...
    Cm.CJS.input <- list(raw.data = dat.1.Cm.site,
    ...
    saveRDS(Cm.results, 
            file = paste0("RData/CJS_Cm_2023_RMark_", site.names.used[k], ".rds"))
    saveRDS(Cm.CJS.input, 
            file = paste0("RData/CJS_Cm_2023_RMark_input_", site.names.used[k], ".rds"))
...
Model comparison is conducted in Cm_CJS_Mark_Jags_site Sep 2024.Rmd
