...
To estimate the long-term population trend and possible deviation from a long-term trend, we fitted a population growth model to the Horvitz-Thompson estimates.  

The time step is not one year. So, we need to make sure that's taken care of. 
...
We can also fit a simple logistic growth model to these estimates.

N(t+1) = N(t) + rN(t)[(K - N(t))/K].

```{r}
data.frame(Nt1 = Nhats.df$Nhat[2:nrow(Nhats.df)],
           Nt = Nhats.df$Nhat[1:(nrow(Nhats.df)-1)],
           time = Nhats.df$time[1:(nrow(Nhats.df)-1)],
           dt = Nhats.df$time[2:(nrow(Nhats.df))] - Nhats.df$time[1:(nrow(Nhats.df)-1)]) %>%
  mutate(dN = Nt1 - Nt,
         dNdt = dN/dt) -> logistic.df

logistic.fit <- nls(dNdt ~ r * Nt * ((K - Nt)/K), 
                    data = logistic.df,
                    start = list(r = 0.18, K = 25000))
...
```{r}
logistic.fit.summary <- summary(logistic.fit)
logistic.df %>%
  mutate(Nt1hat = Nt + logistic.fit.summary$coefficients[1,1] * 
           Nt * ((logistic.fit.summary$coefficients[2,1] - Nt)/logistic.fit.summary$coefficients[2,1])) -> logistic.df.1

#logistic.df$predict <- predict(logistic.fit)

ggplot(data = logistic.df.1) +
...
I decided to run the best model using JAGS. See Cm_CJS_phiTSM_pt_JAGS.Rmd.
