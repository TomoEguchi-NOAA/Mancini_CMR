...
```{r Nhats_exp, echo=FALSE, include=F, cache=T}
Nhats.df %>% 
...
data.vector <- as.vector(jags.data$jags.data$N) %>% 
  rep(each = MCMC.params$n.chains * n.per.chain)

# START HERE ON 2020-09-04 - LOGLIK IS NOT THE RIGHT FORMAT TO COMPUTE LOOIC
...
Nhats.df %>% mutate(N.predict = exp.mu_N.df$mean[1] * exp(exp.r$mean[1] * time)) -> Nhats2.df

p.exp.fit <- ggplot(data = Nhats2.df) +
  geom_path(aes(x = time, y = Nhat),
            color = "blue") +
  geom_path(aes(x = time, y = N.predict),
            color = "orange")
...
```{r plot_linear_Nhats, echo=FALSE, cache=TRUE, fig.cap="Figure 6. The fitted exponential model to estimated abundance of green turtles"}
knitr::include_graphics(paste0("figures/Cm_Nhats_exp.png"))

```

```{r residuals_linear, echo=FALSE, include=F, cache=T}
Nhats2.df %>% mutate(residual = Nhat - N.predict) -> Nhats2.df

p.residual.exp <- ggplot(data = Nhats2.df) + 
  geom_point(aes(x = time, y = residual))
...
```{r plot_linear_residuals, echo=FALSE, cache=TRUE, fig.cap="Figure 7. Residuals of the exponential model fitted to estimated abundance of green turtles"}
...
MCMC.params <- list(n.samples = 50000,
                    ...
                    n.thin = 5,
                    n.chains = 5,
                    model.file = "models/Model_continuous_time_logistic.txt")
...
When the fit of these two models were compared, the deviance information criteria (DIC) indicated that the logistic model was better than the exponential model (delta DIC = ```r jm.exp$DIC - jm.logistic$DIC```). 

Trace plots look okay:

```{r trace_plot_logistic, echo=FALSE, include=T, cache=T}
mcmc_trace(jm.logistic$samples, c("K", "r", "sd_N"))
```

Compare

Summary of K and r are here:

```{r summary_logistic, echo=F, include=T}
jm.logistic$summary %>% data.frame() %>%
  rownames_to_column(var = "Parameter")  -> jm.out.df

post_K <- jm.out.df %>% filter(str_detect(Parameter, "K"))
post_r <- jm.out.df %>% filter(str_detect(Parameter, "r"))

mcmc_dens(jm.logistic$samples, c("K", "r"))
```

The logistic model was considered better than the exponential model based on the DIC, where 

K is quite large (mean = ```r format(signif(jm.logistic$mean$K, 6), scientific = F)```, SE = ```r signif(jm.logistic$sd$K, 3)```) and the estimated r (mean = ```r signif(jm.logistic$mean$r, 3)```, SE = ```r format(signif(jm.logistic$sd$r, 3), scientific = F)```) is much greater than what was found from the exponential model (mean = ```r signif(jm.exp$mean$r, 3)```, SE = ```r format(signif(jm.exp$sd$r, 3), scientific=F)```). 


```{r plot_logistic, echo=F, include=F}
sd_N <- jm.out.df %>% filter(str_detect(Parameter, "sd_N"))

jm.out.df %>% filter(str_detect(Parameter, "mu_N")) %>% # -> jm.out.muN.df # 
  mutate(time = Nhats.df$time[2:nrow(Nhats.df)],
         Nlow = mean - 1.96 * sd_N$mean,
         Nhigh = mean + 1.96 * sd_N$mean) -> jm.out.muN.df

p.logistic.fit <- ggplot() + 
  geom_path(data = Nhats.df,
            aes(x = time, y = Nhat),
            color = "darkblue") + 
  geom_path(data = jm.out.muN.df,
            aes(x = time, y = mean),
            color = "orange") + 
  # geom_point(data = jm.out.muN.df,
  #           aes(x = time, y = mean)) +
  geom_ribbon(data = jm.out.muN.df,
              aes(x = time,
                  ymin = X2.5., 
                  ymax = X97.5.),
              fill = "orange",
...
```{r plot_Nhats_logistic, echo=FALSE, cache=TRUE, fig.cap="Figure 8. Logistic model fitted to estimated abundance of green turtles"}
