...
MCMC.n.chains <- ifelse(n.cores < 5, n.cores, 5)
...
if (!file.exists(paste0("RData/CJS_pdot_Ei_stan.rds"))){
  ## parameters to monitor - when this is changed, make sure to change
  ## summary statistics indices at the end of this script. 
  parameters <- c("mean_p", "mean_phi")
  
  stan.pdot <- stan(file = "models/Model_CJS_p_dot_phi_dot.stan", 
                    data = stan.data,
                    pars = parameters,
                    chains = MCMC.n.chains, 
                    warmup = MCMC.n.burnin,
                    iter = MCMC.n.samples,
                    cores = getOption("mc.cores", MCMC.n.chains),
                    control = list(adapt_delta = 0.99))
    
  out.list <- list(stan.out = stan.pdot,
                   stan.data = stan.data)
  
  saveRDS(out.list, file = paste0("RData/CJS_pdot_phidot_Ei_stan.rds"))
