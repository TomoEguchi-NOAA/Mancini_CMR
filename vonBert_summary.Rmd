...
Some are not very good 95.8% of data were ok or good. So, moving on with the model. 
...
LOOIC.RLinf_Rk.Ei <- readRDS("RData/LOO_Rlinf_Rk_Ei.rds")
LOOIC.RLinf_Fk.Ei <- readRDS("RData/LOO_Rlinf_Fk_Ei.rds")
LOOIC.FLinf_Rk.Ei <- readRDS("RData/LOO_Flinf_Rk_Ei.rds")
LOOIC.FLinf_Fk.Ei <- readRDS("RData/LOO_Flinf_Fk_Ei.rds")

LOOIC.DIC.Ei <- data.frame(model = c("RLinf_Rk", "RLinf_Fk", "FLinf_Rk", "FLinf_Fk"),
                           LOOIC = c(LOOIC.RLinf_Rk.Ei$loo.out$estimates["looic", "Estimate"],
                                     LOOIC.RLinf_Fk.Ei$loo.out$estimates["looic", "Estimate"],
                                     LOOIC.FLinf_Rk.Ei$loo.out$estimates["looic", "Estimate"],
                                     LOOIC.FLinf_Fk.Ei$loo.out$estimates["looic", "Estimate"]),
                           SE = c(LOOIC.RLinf_Rk.Ei$loo.out$estimates["looic", "SE"],
                                  LOOIC.RLinf_Fk.Ei$loo.out$estimates["looic", "SE"],
                                  LOOIC.FLinf_Rk.Ei$loo.out$estimates["looic", "SE"],
                                  LOOIC.FLinf_Fk.Ei$loo.out$estimates["looic", "SE"]),
                           DIC = c(jm.RLinf_Rk.Ei$DIC, 
                                   jm.RLinf_Fk.Ei$DIC, 
                                   jm.FLinf_Rk.Ei$DIC, 
                                   jm.FLinf_Fk.Ei$DIC),
                           max.Rhat = c(max(jm.RLinf_Rk.Ei$summary %>% as.data.frame() %>% select(Rhat)),
                                        max(jm.RLinf_Fk.Ei$summary %>% as.data.frame() %>% select(Rhat)),
                                        max(jm.FLinf_Rk.Ei$summary %>% as.data.frame() %>% select(Rhat)),
                                        max(jm.FLinf_Fk.Ei$summary %>% as.data.frame() %>% select(Rhat)))) %>%
  arrange(by = LOOIC)

LOOIC.DIC.Ei
```

LOOIC indicated , fixed Linf and random k parameter model was considered best for green turtles.  Take a look at some posteriors to see if convergence was okay.  According to the Rhat statistics, they were okay.

```{r}
mcmc_trace(jm.FLinf_Rk.Ei$samples, c("Linf", "Shape", "rate"))
```
...
```{r}
mcmc_dens(jm.FLinf_Rk.Ei$samples, c("Linf", "Shape", "rate"))
```


Next, I plot the fitted von Bertalanffy function. 
```{r}
Linf.samples <- extract.samples("Linf", jm.FLinf_Rk.Ei$samples)
...
t <- i <- 1
for (i in 1:length(unique.ID)){
  A.samples <- extract.samples(paste0("A[", i, "]"), jm.FLinf_Rk.Ei$samples)
  k.samples <- extract.samples(paste0("k[", i, "]"), jm.FLinf_Rk.Ei$samples)
  for (t in 1:length(time.vec)){
    L_Exp <-  Linf.samples * (1.0 - exp(-k.samples * (A.samples + time.vec[t])))
    L_Exp.med[t,i] <- quantile(L_Exp, 0.5)
    L_Exp.lo[t,i] <- quantile(L_Exp, 0.025)
